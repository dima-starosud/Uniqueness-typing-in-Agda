...
pipe {ℓ} {t-in₁} {t-out₁} {t-in₂} {t-out₂} {nr-in₁} {nr-out₁} {nr-in₂} {nr-out₂} tr₁ tr₂ pr-t nr-in nr-out = tr where
    ...
    tr v nr-v t-ì⊆v nr-ò = w , w≋out where
        n-in = names t-in
        v̀ = filter-∈ v n-in
        ...
        w-all₁ : Σ[ w ∶ Values ℓ ] types w ≋ types v̀ ∖∖ names t-in₁ ∪ t-out₁
        w-all₁ = tr₁ v̀ nr-v̀ i₁⊆v̀ nr-v̀∖i₁∪o₁
        ...
        w-all₂ : Σ[ w ∶ Values ℓ ] types w ≋ types w₁ ∖∖ names t-in₂ ∪ t-out₂
        w-all₂ = tr₂ w₁ nr-w₁ i₂⊆w₁ nr-w₁∖i₂∪o₂
...
extract {n = n} {A = A} {nr!-t = nr!-t} tr =
  let e = n , Pure A
      v , t-v≋t = (tr [] [] (≋⇒⊆ refl) (s-nr!⇒nr nr!-t) ∶ (Σ[ v ∶ Values _ ] types v ≋ [ e ]))
